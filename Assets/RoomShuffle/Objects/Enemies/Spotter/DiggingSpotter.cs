...
public class DiggingSpotter : EnemyScript
{
    ...
    private int _relativeSignToPlayer = 0;
    ...
    void Update()
    {
        ...
        switch (_spotPlayer.State)
        {
            ...
            case SpotterPlayerRelationship.Chasing:
                ...
                if (FlipToPlayer() && !_aboveGround && (_relativeSignToPlayer != Math.Sign(_player.Value.transform.position.x - transform.position.x)))
                {
        ...
        _relativeSignToPlayer = Math.Sign(_player.Value.transform.position.x - transform.position.x);
    }
